...
| TD-2026-015 | M2 本地诊断预览与下载 | 进行中 | M2 | 未发布 |
| TD-2026-016 | M2 阅读控制、多模型与译文样式 | 进行中 | M2 | 未发布 |
| TD-2026-017 | M2 可读性、分模型用量与设置体验 | 进行中 | M2 | 未发布 |
...
| 字段 | 内容 |
| --- | --- |
| 状态 | 进行中，代码与自动化契约已完成；Chrome 安装态成功路径待复验 |
| 开始日期 | 2026-08-18 |
| 所属阶段 | M2 |
...
| 字段 | 内容 |
| --- | --- |
| 状态 | 进行中，代码、单元测试与生产构建已完成；Chrome 安装态成功路径待复验 |
| 开始日期 | 2026-08-18 |
| 所属阶段 | M2 |
...
| 字段 | 内容 |
| --- | --- |
| 状态 | 进行中，代码、发布检查和独立视觉验收已完成；Chrome 安装态主链路待可用环境复验 |
| 开始日期 | 2026-08-18 |
| 所属阶段 | M2 |
...
- 项目所有者授权在 `main` 创建一次开发阶段检查点 commit；该动作不创建 tag、不 push、不创建 GitHub Release，也不表示第一版已达到可上线开源状态。
...
遗留与下一步：在可启动 MV3 Service Worker 的 Chrome 环境验证红色偏好落在相近背景时自动回退、标签模型保存/切换和分模型用量图；通过后可关闭 TD-2026-015/016/017 的共同安装态缺口。之后进入 TD-2026-018 的受控公开页面周期回归与 M2 已知问题收口。
